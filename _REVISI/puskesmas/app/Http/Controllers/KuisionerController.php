...
class KuisionerController extends Controller
{
    ...
    public function hasilSRQ(Request $request)
    {
        ...
        // nilai perulangan sesuai index soal//
        // Buat perhitungan total
        foreach ($request->radio as $index => $value) {
            if ($index <= 19) {
                $total_psikologis += $request->radio[$index];
            }
            if ($index === 20) {
                $total_narkoba += $request->radio[$index];
            }
            if ($index >= 21 && $index <= 23) {
                $total_psikotik += $request->radio[$index];
            }
            if ($index >= 24) {
                $total_ptsd += $request->radio[$index];
            }
            $total += $request->radio[$index];
        }

        // keterangan hasil perhitungan index di atas//
        ...
        // jika terdapat satu atau lebih kondidi yang benar maka jawaban abnormal//
    ...
    public function hasilSDQ(Request $request)
    {
        ...
        // perulangan untuk mendapatkan skor//
        for ($x = 0; $x <= 24; $x++) {
        ...
        // cara untuk mendapatkan kesulitan dan skor keseluruhan//
